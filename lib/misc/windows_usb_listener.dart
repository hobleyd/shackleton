// Monitor USB drive insertion and removal events.
// Lifted directly from the pub.dev win32 examples code. Thanks, guys!
  ...
  int mainWindowProc(int hWnd, int msg, int wParam, int lParam) {
    switch (msg) {
      case WM_DEVICECHANGE:
        if (wParam case DBT_DEVICEARRIVAL || DBT_DEVICEREMOVECOMPLETE when lParam != 0) {
          ...
          if (dbhdr.ref.dbch_devicetype == DBT_DEVTYP_VOLUME) {
            ...
            mainSendPort.send({
              'event': wParam == DBT_DEVICEARRIVAL ? 'USB drive connected' : 'USB drive disconnected',
              'drive': driveLetter,
              'info': driveInfo,
            });
          }
